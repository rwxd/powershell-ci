...
.PARAMETER Severity
    Option to use different severity levels
...
param ( 
    ...
    [array]$ExcludeAnalyzerRules = @("PSUseApprovedVerbs"),
...
if ($Linting) {
    Install-Dependency -Name "PSScriptAnalyzer"
    Run-Linting -Path $path -exclude_rules $ExcludeAnalyzerRules -severity $Severity
}
if ($Pester) {
    Install-Dependency -Name "Pester"
    Run-Tests -Path $path
